...
models_module_path = './core/models/deep/han'
...
outputDirectoryPath = "./results/models/han/"
...
ds_train = CustomDataset(df_train, emb_file, max_sent_length=225, max_doc_length=2350)
ds_val = CustomDataset(df_val, emb_file, max_sent_length=225, max_doc_length=2350)
ds_test = CustomDataset(df_test, emb_file, max_sent_length=225, max_doc_length=2350)
loader = torch.utils.data.DataLoader(ds_train, batch_size=batch_size, sampler=sampler, collate_fn=collate_fn, pin_memory=True, shuffle=False)
vloader = torch.utils.data.DataLoader(ds_val, batch_size=batch_size, collate_fn=collate_fn, pin_memory=True, shuffle=False)
tloader = torch.utils.data.DataLoader(ds_test, batch_size=batch_size, collate_fn=collate_fn, pin_memory=True, shuffle=False)
weights = get_pretrained_weights(emb_file, ds_train.vocab, embedding_length).to(device)

# Define model
model = HierarchicalAttentionNetwork(num_classes=mlb.getTrainingStatistics()[0], vocab_size=len(ds.vocab), embed_dim=embedding_length, word_gru_hidden_dim=200,
                                     sent_gru_hidden_dim=200, word_gru_num_layers=1, sent_gru_num_layers=1,
                                     word_att_dim=200, sent_att_dim=200, use_layer_norm=True, dropout=0.2).to(device)
...
for epoch in range(0, num_epochs):      
   ...
   model.train()
   nb = len(loader)
   for batch_idx, (docs, labels, doc_lengths, sent_lengths) in enumerate(loader):
      if batch_idx % 50 == 0:
      ...
      sent_lengths = sent_lengths.to(device)  # (batch_size, padded_doc_length)
      ...
      pred, word_att_weights, sentence_att_weights = model(docs, doc_lengths, sent_lengths)
      opt.zero_grad()
   ...
   for batch_idx, (docs, labels, doc_lengths, sent_lengths) in enumerate(vloader):
      with torch.no_grad():
         docs = docs.to(device)  # (batch_size, padded_doc_length, padded_sent_length)
         labels = labels.type(torch.FloatTensor).to(device)  # (batch_size)
         sent_lengths = sent_lengths.to(device)  # (batch_size, padded_doc_length)
...
for batch_idx, (docs, labels, doc_lengths, sent_lengths) in enumerate(tloader):
   with torch.no_grad():
      docs = docs.to(device)  # (batch_size, padded_doc_length, padded_sent_length)
      labels = labels.type(torch.FloatTensor).to(device)  # (batch_size)
      sent_lengths = sent_lengths.to(device)  # (batch_size, padded_doc_length)
