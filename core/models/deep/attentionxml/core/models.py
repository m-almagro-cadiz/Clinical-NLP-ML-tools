    ...
    def forward(self, inputs, targets, weights=None):
        if self.logits:
            if weights is not None:
                BCE_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none', pos_weight=weights)
            else:
                BCE_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
        else:
            if weights is not None:
                BCE_loss = F.binary_cross_entropy(inputs, targets, reduction='none', weight=weights)
            else:
...
class Model(object):
    ...
    """
    def __init__(self, network, model_path, gradient_clip_value=5.0, device_ids=None, loss_fn='bce', weights=None, **kwargs):
        self.model = nn.DataParallel(network(**kwargs).cuda(), device_ids=device_ids)
        if loss_fn == 'bce':
            self.loss_fn = nn.BCEWithLogitsLoss()
        else:
            self.loss_fn = FocalLoss(logits=True, reduce=True)
    ...
    def train_step(self, train_x: torch.Tensor, train_y: torch.Tensor):
        ...
        if self.weights is not None:
            weights = torch.Tensor(self.weights).to('cuda')
            loss = self.loss_fn(scores, train_y, weights)
        else:
...
class XMLModel(Model):
    ...
    def train_step(self, train_x: Tuple[torch.Tensor, torch.Tensor], train_y: torch.Tensor):
        ...
        if self.weights is not None:
            candidates_aux = np.asarray(candidates)
            weights = torch.Tensor(self.weights[candidates_aux]).to('cuda')
            loss = self.loss_fn(scores, train_y, weights)
        else:
            loss = self.loss_fn(scores, train_y)
        loss.backward()
