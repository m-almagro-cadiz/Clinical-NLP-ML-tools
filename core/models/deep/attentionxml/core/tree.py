...
from deepxml.data_utils import get_word_emb, get_mlb
...
class FastAttentionXML(object):
    ...
    def __init__(self, labels_num, data_cnf, model_cnf, tree_id=''):
        ...
        self.emb_init, self.vocab_size, self.level = None, data_cnf['embedding']['vocab_size'], model_cnf['level']
    ...
    def train_level(self, level, train_x, train_y, valid_x, valid_y):
        ...
        if level == 0:
            ...
            #sums_i, sums_j = np.ravel(train_y.sum(axis=0)), np.ravel(train_y.sum(axis=1))
            #weights_ = [sums_i[(train_y[i,:] == 1).toarray()[0]].sum()/sums_j[i] if sums_j[i] > 0 else 0 for i in range(train_y.shape[0])]
            #sampler = torch.utils.data.sampler.WeightedRandomSampler(torch.DoubleTensor(weights_), train_y.shape[0])
            train_loader = DataLoader(MultiLabelDataset(train_x, train_y),
                                      model_cnf['train'][level]['batch_size'], num_workers=2, shuffle=False)#, sampler=sampler)
            valid_loader = DataLoader(MultiLabelDataset(valid_x, valid_y, training=False),
                                      model_cnf['valid']['batch_size'], num_workers=2)
            #class_weights = np.asarray([train_y.shape[0]/sums_i[j] if sums_i[j] > 0 else 0 for j in range(train_y.shape[1])])
            model = Model(AttentionRNN, labels_num=labels_num, model_path=F'{self.model_path}-Level-{level}',
                          emb_init=self.emb_init, vocab_size=self.vocab_size, loss_fn='bce', weights=None, **data_cnf['model'], **model_cnf['model'])
            ...
            train_loader = DataLoader(MultiLabelDataset(train_x), model_cnf['valid']['batch_size'], num_workers=2)#, sampler=sampler)
            ...
            valid_loader = DataLoader(XMLDataset(valid_x, valid_y, training=False, labels_num=labels_num,
                                      ...
                                      model_cnf['valid']['batch_size'], num_workers=2)
            #class_weights = np.asarray([train_y.shape[0]/sums_i[j] if sums_i[j] > 0 else 0 for j in range(train_y.shape[1])])
            model = XMLModel(network=FastAttentionRNN, labels_num=labels_num, emb_init=self.emb_init, vocab_size=self.vocab_size,
                             model_path=F'{self.model_path}-Level-{level}', loss_fn='bce', **data_cnf['model'], **model_cnf['model'])
            if not os.path.exists(model.model_path):
            ...
            train_loader = DataLoader(XMLDataset(train_x, labels_num=labels_num,
                                                 groups=last_groups, group_labels=group_labels,
                                                 group_scores=group_scores),
                                      model_cnf['valid']['batch_size'], num_workers=2)#, sampler=sampler)
            return train_y, model.predict(train_loader, k=self.top), model.predict(valid_loader, k=self.top)
    ...
    def predict_level(self, level, test_x, k, labels_num):
        ...
        if level == 0:
            ...
            if model is None:
                model = Model(AttentionRNN, labels_num=labels_num, model_path=F'{self.model_path}-Level-{level}',
                              emb_init=self.emb_init, vocab_size=self.vocab_size, **data_cnf['model'], **model_cnf['model'])
            ...
            test_loader = DataLoader(XMLDataset(test_x, labels_num=labels_num,
                                     ...
                                     model_cnf['predict']['batch_size'], num_workers=2)
            return model.predict(test_loader, k=k)
